    ...
    def give_ref_action_aux(self,currentIdx,currentChildIdx,currentGraph,ref_graph):
        ...
        def need_swap(idx,idx_p,cg,gc,rg):
            ...
            try:
                ...
                print("inferred abt node cur_head_p not in ref graph!", file=sys.stderr)
                return None
        ...
        if currentIdx in goldNodeSet:
            ...
            if currentChildIdx:
                ...
                if currentChildIdx in goldNodeSet:
                    ...
                    if parents_to_attach:
                        ...
                        if self.verbose > 1: print("Current child node %s doesn't have parents in gold span graph!"%(currentChildIdx), file=sys.stderr)
            ...
            if currentChildIdx:
                ...
                if currentChildIdx in goldNodeSet:
                    ...
                    if self.verbose > 1: print("Current child node %s should have been deleted!"%(currentChildIdx), file=sys.stderr)
                    return {'type':NEXT1},None 
            else:
                # here currentNode.children must be empty
                if currentNode.children and self.verbose > 1: print("Unaligned node %s has children"%(currentNode.start), file=sys.stderr)
...
class DetOracleSC(Oracle):
    ...
    def give_ref_action(self,state,ref_graph):
        ...
        currentGraph = state.A
        goldNodeSet = list(ref_graph.nodes.keys())
        ...
        if currentIdx in goldNodeSet:
            ...
            if currentChildIdx:
                if currentChildIdx in goldNodeSet:
                    ...
                    if currentIdx in goldChild.children and \
                       currentChildIdx in goldNode.children:
                        if self.verbose > 1: print("Circle detected in gold graph!", file=sys.stderr)
                        gold_edge = ref_graph.get_edge_label(currentIdx,currentChildIdx)
                        return {'type':NEXT1}, gold_edge # next
                        #result_act_type = {'type':NEXT1}
                        #result_act_label = gold_edge
