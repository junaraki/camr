...
class GraphState(object):
        ...
        if self.cidx and self.cidx != START_ID:
            ...
            if isinstance(self.cidx,int) and isinstance(self.idx,int):
                ...
                if self.A.nodes[self.idx].end - self.A.nodes[self.idx].start > 1:
                    path_pos_str = [(GraphState.sent[i]['pos'],GraphState.sent[i]['rel']) for i in path[1:-1] if i not in list(range(self.A.nodes[self.idx].start,self.A.nodes[self.idx].end))]
                    path_x_str_pp = [('X','X') if not isprep(GraphState.sent[i]) else GraphState.sent[i]['form'] for i in path[1:-1] if i not in list(range(self.A.nodes[self.idx].start,self.A.nodes[self.idx].end))]
                else:
            ...
            if parent_to_attach is not None:
                ...
                #path_x_str=[(GraphState.sent[i]['pos'],GraphState.sent[i]['rel']) for i in path[1:-1]]
                    if self.A.nodes[parent_to_attach].end - self.A.nodes[parent_to_attach].start > 1:                
                        apath_x_str = [('X','X') for i in path[1:-1] if i not in list(range(self.A.nodes[parent_to_attach].start,self.A.nodes[parent_to_attach].end))]
                        apath_pos_str = [(GraphState.sent[i]['pos'],GraphState.sent[i]['rel']) for i in path[1:-1] if i not in list(range(self.A.nodes[parent_to_attach].start,self.A.nodes[parent_to_attach].end))]
                        apath_pos_str_pp = [(GraphState.sent[i]['pos'],GraphState.sent[i]['rel']) if not isprep(GraphState.sent[i]) else GraphState.sent[i]['form'] for i in path[1:-1] if i not in list(range(self.A.nodes[parent_to_attach].start,self.A.nodes[parent_to_attach].end))]
                    else:
    ...
    def get_gold_tag_graph(self):
        ...
        for nid in list(gold_tag_graph.nodes.keys())[:]:
    ...
    def evaluate(self):
        ...
        if self.verbose > 1:
            print('Parsed tuples:'+str(parsed_tuples), file=sys.stderr) 
    ...
    def _fix_prop_feature(self,idx,cidx):
        ...
        if isinstance(idx,int) and isinstance(cidx,int):
            ...
            ctok['pred'].update(dict((k,v) for k,v in list(tok.get('pred',{}).items()) if k!=cidx))
            ...
            ctok['args'].update(dict((k,v) for k,v in list(tok.get('args',{}).items()) if k!=cidx))
    ...
    def get_parsed_amr(span_graph):
        ...
        elif len(amr.roots) == 0 and len(list(amr.keys())) != 0:
            foo =  amr[FAKE_ROOT_VAR]
            amr.node_to_concepts[FAKE_ROOT_VAR] = FAKE_ROOT_CONCEPT
