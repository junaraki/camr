...
class Aligner():
        ...
        while unmatched_variables:
            ...
            if match:
                ...
                if rule_type == 'NameEntity':
                    NE_items = [v[0] for k,v in list(amr[matched_variable].items())]
                    ...
                    for sid, n in zip(list(range(span[0],span[1])), NE_items):
                    ...
                    unit = ''
                    unit_node = None
                    for k,v in list(amr[matched_variable].items()):
                        if k == 'quant':
                            quantity = v[0]
                        elif k == 'unit':
                    ...
                    for sid, n in zip(list(range(span[0],span[1])), TEMP_items):
                        alignment[n].append(sid)
                    alignment[matched_variable].append(matched_variable)
    
                    self.remove_aligned_concepts(unmatched_variables,list(amr[matched_variable].items()))
                    ...
                    if span:
                        ...
                        print('WARNING: Variable %s/%s cannot be aligned'%(matched_variable,cur_concept), file=sys.stderr)
                        alignment[matched_variable].append(matched_variable)
                        update = False
                else:
                    pass
                ...
                # update
                if update:
                    tokens = [(i,tk) for i,tk in tokens if i not in list(range(span[0],span[1]))]
                    sent = ' '.join(x for i,x in tokens)
                    if self.verbose > 2:
                        print("Concept '%s' Matched to span '%s' "%(cur_concept,' '.join(w for i,w in enumerate(sentence.split()) if i+1 in range(span[0],span[1]))), file=sys.stderr)
                        print(sent)
                        print(alignment)
    ...
    def span_align(self,sentence,amr):
        ...
        while unmatched_vars:
            ...
            if match:
                ...
                update = True
                if rule_type == "NameEntity":
                    NE_items = [v[0] for k,v in list(amr[cur].items())]
                    ...
                    if quantity and unit:
                        ...
                        self.remove_aligned_concepts(unmatched_vars,list(amr[cur].items()))
                elif rule_type == "NegPolarity":
                    aligned = False
                    for i,token in tokens:
                        if token.lower() in Aligner.neg_polarity:
                    ...
                    if span:
                        ...
                        print('Variable/Concept %s/%s cannot be aligned'%(cur,cur_concept), file=sys.stderr)
                        #alignment[matched_variable].append(matched_variable)
                        update = False                    
            else:
            ...
            # update
            if update:
                tokens = [(i,tk) for i,tk in tokens if i not in list(range(span.start,span.end))]
                sent = ' '.join(x for i,x in tokens)
                if self.verbose > 2:
                    print("Concept '%s' Matched to span '%s' "%(cur_concept,' '.join(w for i,w in enumerate(sentence.split()) if i+1 in range(span[0],span[1]))), file=sys.stderr)
                    print(sent)
    ...
    def align_single_concept(self,sent,tokens,cur_var,amr,alignment,unmatched_vars,triples,NEXT=False):
        ...
        if match:
            ...
            update = True
            if rule_type == "NameEntity":
                NE_items = [v[0] for k,v in list(amr[cur_var].items()) if isinstance(v[0],StrLiteral)]
                ...
                for k,v in list(amr[cur_var].items()):
                ...
                if aligned:
                    ...
                    print('Variable/Concept %s/%s cannot be aligned'%(cur_var,cur_concept), file=sys.stderr)
                    update = False
                ...
                if aligned:
                    ...
                    print('(%s/%s) :op1 (%s/%s) cannot be aligned'%(cur_var,cur_concept,k_var,amr.node_to_concepts[k_var]), file=sys.stderr)
                    update = False
                ...
                if aligned:
                    ...
                    print('(%s/%s) :op1 (%s/%s) cannot be aligned'%(cur_var,cur_concept,arg0_var,amr.node_to_concepts[arg0_var]), file=sys.stderr)
                    update = False
            else:
                pass
            ...
            if rule_type == "SingleConcept":
                ...
                print('Can not find type of concept %s / %s'%(cur_var,cur_concept), file=sys.stderr)
        ...
        if m:                        
            ...
            if m.group()[0] == ' ' and m.group()[-1] == ' ':
                spans = [(tokens[i][0],tokens[i][0]+len(items)) for i in range(len(tokens)) \
                         if (sum([len(x[1])+1 for x in tokens[:i]])-1,sum([len(x[1])+1 for x in tokens[:i]])-1+len(m.group())) == m.span()]
            else:
                spans = [(tokens[i][0],tokens[i][0]+len(items)) for i in range(len(tokens)) \
                         if (sum([len(x[1])+1 for x in tokens[:i]]),sum([len(x[1])+1 for x in tokens[:i]])+len(m.group())) == m.span()]
            return spans[0]
        else:
            print(pattern.pattern)
            print(sent)
