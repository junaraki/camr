...
class SpanGraph(object):
    ...
    def init_ref_graph(amr,alignment,sent=None):
        ...
        for h in varables:
            ...
            for edge,ds in list(amr[h].items()):
                d = ds[0]
        ...
        if amr.roots[0] in span_alignment:
            ...
            if debuglevel > 1:
                print("GraphID:%s WARNING:root %s not aligned!"%(SpanGraph.graphID,amr.node_to_concepts[amr.roots[0]]), file=log)
    ...
    def pre_merge_netag(self,instance):
        ...
        for ne_id,span in list(ne_spans.items()):
            for sp_id in span:
